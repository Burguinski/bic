...
function beaz_get_custom_fields ($type) {
 if($type == 'empleo') return beaz_get_empleo_custom_fields();
  else return [];
}

function beaz_show_custom_fields() { //Show box
  global $post;
  $type = get_post_type($post->ID);
  $fields = beaz_get_custom_fields ($type); ?>
        ...
        <?php if(!isset($datos['is']) || (isset($datos['is']) && has_term($datos['is']['id'], $datos['is']['taxonomy'], $post->ID))) { ?>
          <?php if($datos['tipo'] != 'gallery' && $datos['tipo'] != 'separator' && $datos['tipo'] != 'links' && $datos['tipo'] != 'positions') { ?><div style="width: calc(50% - 10px); float: left; padding: 5px;"><?php } else { ?><div style="width: calc(100% - 10px); float: left; padding: 5px;"><?php } ?>
            ...
            <?php } else if($datos['tipo'] == 'image') { ?>
              ...
              <a href="#" id="button_media_<?php echo $field; ?>" class="button insert-media add_media" data-editor="input_<?php echo $type; ?>_<?php echo $field; ?>" title="<?php _e("Add file", 'beaz'); ?>"><span class="wp-media-buttons-icon"></span> <?php _e("Add file", 'beaz'); ?></a>
              <script>
...
<!--Tipo PDF-->
            <?php } else if($datos['tipo'] == 'pdf') {
                      $pdfs = get_post_meta( $post->ID, '_empleo_'.$field, true );
                      if (!is_array($pdfs)) {
                          $pdfs = array();
                      }
                      ?>
                      <div class="pdf_repeater_<?php echo $field; ?>">
                          <?php if (empty($pdfs)) { ?>
                              <div class="pdf_item" style="margin-bottom: 10px; padding: 8px; border:1px solid #eee;">
                                <input type="text" name="_empleo_<?php echo $field; ?>_titulo[]" class="pdf_titulo" style="width: 35%; margin-bottom: 5px;" placeholder="Título para el documento" value="" /></br>
                                  <input type="text" name="_empleo_<?php echo $field; ?>_url[]" class="pdf_url" style="width: 45%;" placeholder="URL del PDF" value="" />
                                  <a href="#" class="button button-add-pdf-media">Seleccionar PDF</a>
                                  <a href="#" class="button button-remove-pdf" style="background: #dc3232; color: white;">Eliminar</a>
                              </div>
                          <?php } else {
                              foreach ($pdfs as $pdf) { 
                                  $url = isset($pdf['url']) ? $pdf['url'] : '';
                                  $titulo = isset($pdf['titulo']) ? $pdf['titulo'] : '';
                                  ?>
                                  <div class="pdf_item" style="margin-bottom: 10px; padding: 8px; border:1px solid #eee;">
                                      <input type="text" name="_empleo_<?php echo $field; ?>_titulo[]" class="pdf_titulo" style="width: 35%; margin-bottom: 5px;" placeholder="Título para el documento" value="<?php echo $titulo; ?>" /></br>
                                      <input type="text" name="_empleo_<?php echo $field; ?>_url[]" class="pdf_url" style="width: 45%;" placeholder="URL del PDF" value="<?php echo esc_attr($url); ?>" />
                                      <a href="#" class="button button-add-pdf-media">Seleccionar PDF</a>
                                      <a href="#" class="button button-remove-pdf" style="background: #dc3232; color: white;">Eliminar</a>
                                  </div>
                              <?php }
                      ...
                      <div style="margin-top: 10px;">
                          <a href="#" class="button button-add-new-pdf" data-field="<?php echo $field; ?>">+ Añadir nuevo PDF</a>
                      </div>
              ...
              <div style=" display: flex; flex-wrap: wrap; gap: 10px;">
                <?php for ($i = 0; $i < $max; $i++) { ?>
                  <div style="width: 200px; text-align: center; padding: 5px;background-color: #cecece; min-height: 245px; position: relative;">
                    <div title="<?php _e("Borrar", 'beaz'); ?>" style="position: absolute; bottom: 0px; right: 0px; padding: 5px 10px; background-color: red; color: white; cursor: pointer;" class="_<?php echo $type; ?>_<?php echo $field; ?>_<?php echo $i; ?>_delete" id="_<?php echo $type; ?>_<?php echo $field; ?>_<?php echo $i; ?>_delete">✕</div>
                    ...
                    <b class='set_custom_gallery_<?php echo $field; ?>_<?php echo $i; ?> button' style="display: block; width: 100%;"><?php _e("Añadir imagen", 'beaz'); ?></b>
               ...
               ?>
                <div class="<?php echo $type; ?>_<?php echo $field; ?>">
                  <?php if(is_array($links)) { array_values($links);
                    ...
                    <div>
                      <b class="handle">&#8597;</b>
                      <input type="text" class="_<?php echo $type; ?>_<?php echo $field; ?>_text_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_text_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][text]" value='<?php echo (isset($links[$i]) ? $links[$i]['text'] : ""); ?>' placeholder="<?php _e("Texto", 'beaz'); ?>" />             
                      <input type="text" class="_<?php echo $type; ?>_<?php echo $field; ?>_link_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_link_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][link]" value='<?php echo (isset($links[$i]) ? $links[$i]['link'] : ""); ?>' placeholder="<?php _e("URL", 'beaz'); ?>" />  
                      <span title="<?php _e("Borrar", 'beaz'); ?>">✕</span>     
                    </div>
                  <?php } } else $i = 0;
                   $i++;?>
                  <div>
                    <b class="handle">&#8597;</b>
                    <input type="text" class="_<?php echo $type; ?>_<?php echo $field; ?>_text_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_text_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][text]" value='' placeholder="<?php _e("Texto", 'beaz'); ?>" />
                    <input type="text" class="_<?php echo $type; ?>_<?php echo $field; ?>_link_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_link_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][link]" value='' placeholder="<?php _e("URL", 'beaz'); ?>" />     
                    <span title="<?php _e("Borrar", 'beaz'); ?>">✕</span>
                  </div>
                </div>
                <a href="#" class="button button-primary" id="new-<?php echo $type; ?>_<?php echo $field; ?>"><?php _e("Nueva publicación", 'beaz'); ?></a>
                <style>
                ...
                <script>
                  ...
                  jQuery("#new-<?php echo $type; ?>_<?php echo $field; ?>").click(function(e) {
                    ...
                    var append = "<div><b class='handle'>&#8597;</b>"+
                      "<input type='text' class='_<?php echo $type; ?>_<?php echo $field; ?>_text_"+counter+"' id='_<?php echo $type; ?>_<?php echo $field; ?>_text_"+counter+"' style='width: 80%;' name='_<?php echo $type; ?>_<?php echo $field; ?>["+counter+"][text]' value='' placeholder='<?php _e("Texto", 'beaz'); ?>'>"+
                      "<input type='text' class='_<?php echo $type; ?>_<?php echo $field; ?>_link_"+counter+"' id='_<?php echo $type; ?>_<?php echo $field; ?>_link_"+counter+"' style='width: 80%;' name='_<?php echo $type; ?>_<?php echo $field; ?>["+counter+"][link]' value='' placeholder='<?php _e("URL", 'beaz'); ?>'>"+   
                      "<span title='<?php _e("Borrar", 'beaz'); ?>'>✕</span></div>";
              ...
              foreach (get_posts($args) as $investigador) {
                ...
                <div class="<?php echo $type; ?>_<?php echo $field; ?>">
                  <?php if(is_array($positions)) { array_values($positions);
                    ...
                    <div>
                      ...
                      <select class="_<?php echo $type; ?>_<?php echo $field; ?>_text_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_text_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][investigador]">
                        <option><?php _e("Elegir miembro del comite", 'beaz'); ?></option>
                      ...
                      <input type="text" class="_<?php echo $type; ?>_<?php echo $field; ?>_position_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_position_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][position]" value='<?php echo (isset($positions[$i]) ? $positions[$i]['position'] : ""); ?>' placeholder="<?php _e("Cargo", 'beaz'); ?>" />
                      <span title="<?php _e("Borrar", 'beaz'); ?>">✕</span>                   
                    </div>
                  <?php } } else $i = 0;
                   $i++;?>
                  <div>
                    ...
                    <input type="text" class="_<?php echo $type; ?>_<?php echo $field; ?>_position_<?php echo $i; ?>" id="_<?php echo $type; ?>_<?php echo $field; ?>_position_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][position]" value='' placeholder="<?php _e("Cargo", 'beaz'); ?>" />     
                    <span title="<?php _e("Borrar", 'beaz'); ?>">✕</span>
                  </div>
                </div>
                <a href="#" class="button button-primary" id="new-<?php echo $type; ?>_<?php echo $field; ?>"><?php _e("Nuevo miembro", 'beaz'); ?></a>
                <style>
                  .<?php echo $type; ?>_<?php echo $field; ?> {
                    display: flex;
                    flex-direction: column;
                ...
                <script>
                  ...
                  jQuery("#new-<?php echo $type; ?>_<?php echo $field; ?>").click(function(e) {
                    ...
                    var append = "<div><b class='handle'>&#8597;</b>"+
                      ...
                      "<input type='text' class='_<?php echo $type; ?>_<?php echo $field; ?>_position_"+counter+"' id='_<?php echo $type; ?>_<?php echo $field; ?>_position_"+counter+"' style='width: 80%;' name='_<?php echo $type; ?>_<?php echo $field; ?>["+counter+"][position]' value='' placeholder='<?php _e("Cargo", 'beaz'); ?>'>"+   
                      "<span title='<?php _e("Borrar", 'beaz'); ?>'>✕</span></div>";
                    jQuery(".<?php echo $type; ?>_<?php echo $field; ?>").append(append);
...
<!--Script de prueba para tipo PDF-->
 <script>
    jQuery(document).ready(function($) {
        // Abrir selector de medios
        $(document).on('click', '.button-add-pdf-media', function(e) {
            e.preventDefault();
            var currentInput = $(this).siblings('.pdf_url');
            var frame = wp.media({
        ...
        // Añadir nuevo PDF (con los dos inputs en el orden correcto)
        $(document).on('click', '.button-add-new-pdf', function(e) {
            e.preventDefault();
            var field = $(this).data('field');
            var newItem = '<div class="pdf_item" style="margin-bottom: 10px; padding: 8px; border:1px solid #eee;">' +
                '<input type="text" name="_empleo_' + field + '_titulo[]" class="pdf_titulo" style="width: 35%; margin-bottom: 5px;" placeholder="Título a mostrar" value="" /></br>' +
                '<input type="text" name="_empleo_' + field + '_url[]" class="pdf_url" style="width: 45%;" placeholder="URL del PDF" value="" />' +
                '<a href="#" class="button button-add-pdf-media">Seleccionar PDF</a>' +
                '<a href="#" class="button button-remove-pdf" style="background: #dc3232; color: white;">Eliminar</a>' +
                '</div>';
            $('.pdf_repeater_' + field).append(newItem);
        });
        
        // Eliminar PDF
        $(document).on('click', '.button-remove-pdf', function(e) {
            e.preventDefault();
            $(this).closest('.pdf_item').remove();
        });
    });
    </script>
<?php }

function beaz_save_custom_fields( $post_id ) { //Save changes
  ...
  $fields = beaz_get_custom_fields ($type);
    ...
    }  elseif ($datos['tipo'] == 'pdf') {
        $urls = isset($_POST['_empleo_'.$field.'_url']) ? $_POST['_empleo_'.$field.'_url'] : array();
        $titulos = isset($_POST['_empleo_'.$field.'_titulo']) ? $_POST['_empleo_'.$field.'_titulo'] : array();
        $pdfs = array();
        for ($i = 0; $i < count($urls); $i++) {
            if (!empty($urls[$i])) {
                $pdfs[] = array(
                    'url' => esc_url_raw($urls[$i]),
                    'titulo' => sanitize_text_field($titulos[$i])
                );
            }
        }
        update_post_meta($post_id, '_empleo_'.$field, $pdfs);
...
// Libs ----------------------------------------
function beaz_sort_terms_hierarchically($terms) {
	usort($terms, "cmp");
	return $terms;
}

function beaz_cmp($a, $b) {
