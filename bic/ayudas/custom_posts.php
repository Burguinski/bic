...
function bic_get_custom_fields ($type) {
  if($type == 'ayuda') return bic_get_ayuda_custom_fields();
  else if($type == 'video') return bic_get_video_custom_fields();
  else if($type == 'documento') return bic_get_documento_custom_fields();
  else if($type == 'espacio') return bic_get_espacio_custom_fields();
  else return [];
}
        ...
        <?php if(!isset($datos['is']) || (isset($datos['is']) && has_term($datos['is']['id'], $datos['is']['taxonomy'], $post->ID))) { ?>
          <?php if($datos['tipo'] != 'gallery' && $datos['tipo'] != 'separator' && $datos['tipo'] != 'links' && $datos['tipo'] != 'positions') { ?><div style="width: calc(50% - 10px); float: left; padding: 5px;"><?php } else { ?><div style="width: calc(100% - 10px); float: left; padding: 5px;"><?php } ?>
            ...
            <?php } else if($datos['tipo'] == 'links') { 
              ...
              <?php
              ...
              } else if($datos['tipo'] == 'tabla_reserva') { 
                $reservas_meta = get_post_meta($post->ID, '_espacio_reserva', true);
                $fechas_reservadas = !empty($reservas_meta) ? json_decode($reservas_meta, true) : []; 
                ?>
               ...
               <table border="1" cellpadding="8" style="border-collapse: collapse; width: 100%;">
                  ...
                  <tbody>
                      <?php 
                      foreach ($fechas_reservadas as $index => $reserva) {
                          $fechas = $reserva['fechas'];
                          $fecha_inicio = date('d-m-Y', strtotime($fechas[0]));
                          $fecha_fin = date('d-m-Y', strtotime(end($fechas)));
                          $total_dias = count($fechas);
                      ?>
                      <tr>
                          <td><?php echo $index + 1;?></td>
                          <td><?php echo $fecha_inicio;?> (Inicio) | <?php echo $fecha_fin;?> (Fin)</td>
                          <td><?php echo $total_dias;?></td>
                          <td>
                              <input type="submit" name="borrar_reserva" value="Eliminar" style="background-color: red; color: white;">
                              <input type="hidden" name="reserva_index" value="<?php echo $index; ?>"><!--enviamos el campo oculto-->
                          </td>
                        </tr>
                      <?php } ?>
                  </tbody>
...
function bic_save_custom_fields( $post_id ) { //Save changes
  ...
  $fields = bic_get_custom_fields ($type);
    ...
    if ($datos['tipo'] == 'tags') {
      ...
      echo "111111111111";
        if (isset($_POST['borrar_reserva']) && isset($_POST['reserva_index'])) {
          echo "2222222222222";
            $indice = intval($_POST['reserva_index']);//obtenemos el int de la variable
            $reservas_meta = get_post_meta(get_the_id(), '_espacio_reserva', true);//cargamos el json en un array
            ...
            if (isset($reservas[$indice])) {
                ...
                unset($reservas[$indice]);//elimina la reserva del array con ese indice
                $reservas = array_values($reservas); //reordenar los indices
                update_post_meta(get_the_id(), '_espacio_reserva', json_encode($reservas, JSON_UNESCAPED_UNICODE));//actualizamos la base de datos
                echo "<p style='color:green;'>Reserva eliminada correctamente.</p>";
            }
        }
