...
function wp_pcc_plugin_menu() {
	add_options_page( __('Incubadas', 'wp-perfil-contacto'), __('Panel de empresas incubadas', 'wp-perfil-contacto'), 'manage_options', 'wp-perfil-contacto', 'wp_pcc_page_settings');
}
...
function wp_pcc_page_settings() { 
	...
	if(isset($_REQUEST['send']) && $_REQUEST['send'] != '') { 
		...
		update_option('_wp_pcc_incubada_page_id', $_POST['_wp_pcc_incubada_page_id']);
		update_option('_wp_pcc_incubada_edit_profile_id', $_POST['_wp_pcc_incubada_edit_profile_id']);
	} ?>
	<form method="post">
    <h2><?php _e("Configuración Principal", 'wp-perfil-contacto'); ?></h2>
		...
		<b><?php _e("ID de la página del perfil de empresa incubada", 'wp-perfil-contacto'); ?>:</b><br/>
		<input type="text" name="_wp_pcc_incubada_page_id" value="<?php echo get_option("_wp_pcc_incubada_page_id"); ?>" style="width: calc(100% - 20px);" /><br/>
		<b><?php _e("ID de la página donde está el editor de empresa incubada", 'wp-perfil-contacto'); ?>:</b><br/>
		<input type="text" name="_wp_pcc_incubada_edit_profile_id" value="<?php echo get_option("_wp_pcc_incubada_edit_profile_id"); ?>" style="width: calc(100% - 20px);" /><br/>
